...
/* ==================================================================
   3e. CROWN INTERACTIONS
       CLICK  — a burst of 4-8 tiny crowns in random sizes spray out
                from the click point and gravity-fall like confetti.
       HOLD   — a single crown inflates like a balloon; on release it
                pops (burst-expand + scatter tiny crowns outward).
       Each crown gets a random brand tint.
   ================================================================== */
if (hero && FINE && !RM) {
  const layer = document.createElement('div');
  layer.className = 'trail-layer'; layer.setAttribute('aria-hidden','true');
  hero.appendChild(layer);

  let live = 0;
  const MAX_LIVE = 30;
  const GRAVITY  = 340;
  const HOLD_THRESHOLD = 260;  /* ms: shorter = click burst, longer = balloon */

  const TINTS = [
    'brightness(1)    saturate(1)',
    'brightness(.72)  saturate(1.3)  sepia(.15)',
    'brightness(.48)  saturate(.9)   sepia(.22)',
    'brightness(1.08) saturate(.5)   sepia(.06)',
    'brightness(.88)  saturate(1.6)  sepia(.10) hue-rotate(4deg)',
    'brightness(.58)  saturate(.7)   sepia(.30)',
    'brightness(1.14) saturate(.35)',
    'brightness(.38)  saturate(.6)   sepia(.18)',
  ];
  let tI = 0;
  const nextTint = () => TINTS[tI++ % TINTS.length];
  const rand = (a,b) => a + Math.random() * (b-a);

  /* ---- shared: spawn a single falling crown at (x,y) with velocity ---- */
  function spawnFalling(x, y, sz, vx0, vy0, tint) {
    if (live >= MAX_LIVE) return;
    const el = document.createElement('div');
    el.className = 'crown-pop';
    el.style.width = sz.toFixed(1) + 'px';
    el.style.filter = tint;
    el.style.opacity = '1';
    layer.appendChild(el);
    live++;

    let px = x, py = y, vx = vx0, vy = vy0;
    let rot = rand(-40, 40), vr = rand(-200, 200);
    let opacity = 1, t0 = performance.now();
    ...
    function tick(now) {
      ...
      if (py > heroH * 0.55) opacity -= dt * 2.6;
      opacity = Math.max(0, opacity);

      el.style.transform =
        `translate(-50%,-50%) translate3d(${px.toFixed(1)}px,${py.toFixed(1)}px,0) ` +
        `rotate(${rot.toFixed(1)}deg) scale(${(0.85 + 0.15 * opacity).toFixed(3)})`;
      el.style.opacity = opacity.toFixed(3);

      if (opacity > 0 && py < heroH + 100) {
  ...
  /* ---- CLICK BURST: spray 4-8 tiny crowns ---- */
  function burst(x, y) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const vy = Math.sin(angle) * speed - rand(100, 200); /* bias upward */
      /* slight position scatter so they don't all start at exact same point */
      const ox = rand(-8, 8), oy = rand(-8, 8);
      spawnFalling(x + ox, y + oy, sz, vx, vy, nextTint());
    }
  }

  /* ---- BALLOON: inflate while holding, pop on release ---- */
  let pressing = false, downT = 0, downX = 0, downY = 0;
  let balloon = null, balloonRaf = 0, balloonSz = 0;

  function startBalloon(x, y) {
    const el = document.createElement('div');
    el.className = 'crown-balloon';
    el.style.filter = nextTint();
    layer.appendChild(el);
    balloon = el;
    balloonSz = 20;

    const rot0 = rand(-10, 10);
    function grow() {
      if (!pressing || balloon !== el) return;
      const hold = performance.now() - downT;
      const p = Math.min(hold / 1200, 1);
      const ep = 1 - Math.pow(1 - p, 3);
      balloonSz = 20 + 110 * ep;

      /* gentle wobble as it inflates — like rubber stretching */
      const wobX = 1 + 0.03 * Math.sin(hold * 0.012) * ep;
      const wobY = 1 + 0.03 * Math.cos(hold * 0.015) * ep;

      el.style.width = balloonSz.toFixed(1) + 'px';
      el.style.opacity = String(Math.min(1, 0.25 + ep * 0.75).toFixed(3));
      el.style.transform =
        `translate(-50%,-50%) translate3d(${x.toFixed(1)}px,${(y - ep * 14).toFixed(1)}px,0) ` +
        `rotate(${(rot0 * (1 - ep * 0.5)).toFixed(1)}deg) scale(${wobX.toFixed(3)},${wobY.toFixed(3)})`;
      balloonRaf = requestAnimationFrame(grow);
    }
    grow();
  }
  ...
  function popBalloon(x, y) {
    ...
    /* POP: the balloon crown bursts — quick expand + fade */
    const sz = balloonSz;
    el.style.transition = 'transform 220ms cubic-bezier(.22,1.4,.36,1), opacity 180ms ease-out';
    el.style.transform =
      `translate(-50%,-50%) translate3d(${x.toFixed(1)}px,${(y - 14).toFixed(1)}px,0) scale(1.6)`;
    el.style.opacity = '0';
    setTimeout(() => el.remove(), 240);

    /* scatter fragments: tiny crowns burst outward from the pop */
    const frags = Math.floor(rand(5, 10));
    for (let i = 0; i < frags; i++) {
      const fsz = rand(10, sz * 0.35);
      const angle = rand(0, Math.PI * 2);
      const speed = rand(120, 320);
      const vx = Math.cos(angle) * speed;
      const vy = Math.sin(angle) * speed - rand(140, 260);
      spawnFalling(x + rand(-6,6), y - 10 + rand(-6,6), fsz, vx, vy, nextTint());
    }
  }

  /* ---- event wiring ---- */
  hero.addEventListener('pointerdown', e => {
    if (e.target.closest('a,button,.fan-card,.deck')) return;
    pressing = true;
    downT = performance.now();
    const r = hero.getBoundingClientRect();
    downX = e.clientX - r.left;
    downY = e.clientY - r.top;
    /* start balloon immediately — if it's a quick click we'll pop it tiny */
    startBalloon(downX, downY);
  });
  ...
  function handleRelease(e) {
    ...
    if (dur < HOLD_THRESHOLD) {
      /* quick click: remove balloon silently, do burst instead */
      if (balloon) { cancelAnimationFrame(balloonRaf); balloon.remove(); balloon = null; }
      burst(downX, downY);
    } else {
      /* long hold: pop the balloon */
      popBalloon(downX, downY);
    }
  }

  hero.addEventListener('pointerup', e => {
    if (e.target.closest('a,button,.fan-card,.deck')) { pressing = false; if(balloon){cancelAnimationFrame(balloonRaf);balloon.remove();balloon=null;} return; }
    handleRelease(e);
  });
  hero.addEventListener('pointerleave', handleRelease);
}
...
;(function(){
  ...
  const BUBBLES = [
    ...
    { img:'data:image/webp;base64,', x:13, y:68, size:42 },
    ...
    { img:'data:image/webp;base64,', x:94, y:42, size:38 },
