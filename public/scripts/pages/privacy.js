/* Behaviour for /privacy, lifted from em-creative-studio-privacy_1.html. */
(() => {
'use strict';
...
/* policy table-of-contents — scrollspy highlight (nearest section above the trigger line) */
...
if (tocLinks.length && tocSections.length) {
  ...
  const update = () => {
    ...
    for (const sec of tocSections) {
      if (sec.getBoundingClientRect().top <= TRIGGER) current = sec;
    }
